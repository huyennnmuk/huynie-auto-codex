...
Usage:
    PYTHONPATH=auto-codex python -m merge.install_hook [--project-path /path/to/project]
"""
...
# Skip if we're in a worktree (auto-codex branches)
if [[ "$BRANCH" == "main" ]] || [[ "$BRANCH" == "master" ]]; then
    ...
    if [[ -d ".git" ]]; then
        ...
        (PYTHONPATH="$PROJECT_ROOT/auto-codex" $PYTHON -m merge.tracker_cli notify-commit "$COMMIT_HASH" 2>/dev/null &) &
