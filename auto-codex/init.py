...
def init_auto_codex_dir(project_dir: Path) -> tuple[Path, bool]:
    ...
    # Migration: if a legacy `.auto-claude/` folder exists, rename it in-place.
    legacy_dir = project_dir / ".auto-claude"
    if not auto_codex_dir.exists() and legacy_dir.exists():
        legacy_dir.rename(auto_codex_dir)
...
def get_auto_codex_dir(project_dir: Path, ensure_exists: bool = True) -> Path:
    """
...
def get_auto_claude_dir(project_dir: Path, ensure_exists: bool = True) -> Path:
    """Deprecated alias for `get_auto_codex_dir` (kept for backwards compatibility)."""
    return get_auto_codex_dir(project_dir, ensure_exists=ensure_exists)
