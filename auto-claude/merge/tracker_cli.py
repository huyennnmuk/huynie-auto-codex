...
def find_project_root() -> Path:
    """Find the project root by looking for .auto-codex or .git directory."""
    current = Path.cwd()

    # Walk up until we find .auto-codex or .git
    while current != current.parent:
        if (current / ".auto-codex").exists() or (current / ".git").exists():
            return current
