    ...
    <div className={cn(
      ...
      {hasGitConflicts && mergePreview.gitConflicts && (
        <div className="mb-3 p-2 bg-warning/10 rounded text-xs border border-warning/30">
          <p className="font-medium text-warning mb-1">分支已偏离，AI 将处理</p>
          <p className="text-muted-foreground mb-2">
            自该 worktree 创建以来，主分支新增了 {mergePreview.gitConflicts.commitsBehind} 个提交。
      ...
      <div className="grid grid-cols-2 gap-2 text-xs text-muted-foreground">
        ...
        {hasGitConflicts ? (
          <div className="text-warning">AI 将解决冲突</div>
        ) : hasAIConflicts ? (
          <>
            <div>可自动合并：{mergePreview.summary.autoMergeable}</div>
            {mergePreview.summary.aiResolved !== undefined && (
              <div>AI 已解决：{mergePreview.summary.aiResolved}</div>
            )}
            {mergePreview.summary.humanRequired !== undefined && mergePreview.summary.humanRequired > 0 && (
              <div className="text-warning">需人工确认：{mergePreview.summary.humanRequired}</div>
            )}
          </>
        ) : (
          <div className="text-success">可合并</div>
        )}
      </div>
    </div>
  );
